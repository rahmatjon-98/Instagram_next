...
const SidebarModal = () => {
  ...
  return (
    <motion.div
      ...
      className={`fixed top-0 left-12 p-5 z-30 h-full w-[400px] ${
        theme == "dark" ? "bg-black" : "bg-[white]"
      } shadow-xl rounded-r-[16px]`}
    >
      <button
        onClick={() => setOpenModal(false)}
        className={`absolute cursor-pointer top-4 right-4 p-2 rounded-full ${
        ...
        <>
          <div className="relative mt-9">
            {!focused && (
              <Search
                size={18}
                className="absolute left-3 top-[25px] -translate-y-1/2 text-gray-500 pointer-events-none"
              />
            )}
            <input
              type="text"
            ...
            <div className="flex mt-4 items-center justify-between">
              <button
                ...
                className={`${
                  searchState ? "underline text-blue-600" : "none"
                } font-semibold`}
              >
                Accounts
              </button>
              <button
                ...
                className={`${
                  locationState ? "underline text-blue-600" : "none"
                } font-semibold`}
              >
                Location
              </button>
          ...
          <div className="py-7 flex justify-between items-center">
            ...
            {!searchHistories?.data?.length <= 0 && (
              <button
                ...
                className="font-medium hover:underline cursor-pointer text-[rgb(125,161,255)]"
              >
                Clear All
              </button>
            )}
          ...
          <div className="flex overflow-y-auto h-[400px] flex-col">
            {!search
              ? searchHistories?.data?.length <= 0 && <p>No recent searches.</p>
              : null}
            {search ? (
              loading ? (
                ...
                filteredUsers.map((e) => (
                  <div onClick={() => addUserHistory(e.id)} key={e.id}>
                    <div
                      onClick={() => linkToProfile(e.id)}
                      className={`flex ${
                        theme == "dark"
                          ? "hover:bg-[#2a2a2a]"
                          : "hover:bg-[#eeeeee] "
                      } items-center rounded p-3 w-full gap-[10px]`}
                    >
                      <Image
                        src={
                          e.avatar
                            ? `http://37.27.29.18:8003/images/${e.avatar}`
                            : defaultUser
                        }
                ...
                <div
                  ...
                  className={`flex ${
                    theme == "dark"
                      ? "hover:bg-[#2a2a2a]"
                      : "hover:bg-[#eeeeee] "
                  } items-center rounded p-3 w-full justify-between`}
                >
                  <div
                    onClick={() => linkToProfile(e?.users.id)}
                    className="flex cursor-pointer items-center gap-5"
                  >
                    <Image
                      src={
                        e?.users?.avatar
                          ? `http://37.27.29.18:8003/images/${e?.users?.avatar}`
                          : defaultUser
                      }
                      className="object-cover w-[44px] h-[44px] rounded-full"
                      width={44}
                  ...
                  <button
                    onClick={() => deleteUserHistory(e.id)}
              ...
              {search2 ? (
                loading2 ? (
                  ...
                  filteredUsers2?.map((e) => (
                    <div
                      className="flex items-center cursor-pointer hover:bg-[#eeeeee] rounded p-3 w-full gap-[10px]"
                      key={e.locationId}
                    >
